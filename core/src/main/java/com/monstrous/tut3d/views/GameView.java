...
public class GameView implements Disposable {
    ...
    public GameView(World world, boolean overlay, float near, float far, float bobScale) {
        ...
        // Fog
//        sceneManager.environment.set(new ColorAttribute(ColorAttribute.Fog, Color.GRAY));
//        sceneManager.environment.set(new FogAttribute(FogAttribute.FogEquation).set(near, far/15, 1f));
    ...
    public void render(float delta, float speed, DecalBatch decalBatch) {
        if(!isOverlay)
            camController.update(world.getPlayer().getPosition(), world.getPlayerController());
        else
