...
//            massInfo.setMass(1);
            try { rigidBody.setMass(massInfo); } catch (RuntimeException e) {}
        ...
        switch(shapeType) {
            ...
            case CAPSULE:
            	if (h <= radius)
            		...
            		CapsuleShapeBuilder.build(meshBuilder, radius, radius * 3, 12);
                break;
            case CYLINDER:
    ...
    public PhysicsBody createBody(ModelInstance collisionInstance, CollisionShapeType shapeType, boolean isStatic, Vector3 dimension) {
        float w = dimension.x;
        float h = dimension.y;
        float d = dimension.z;
